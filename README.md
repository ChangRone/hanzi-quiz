...
本文件以目前**最新完整專案包**的實際內容為基準整理；本次已把題庫索引調整為「`quiz-index.json` 一教材一筆 + `lessonPattern` / `lessonRange` + `packs/` 每課單檔」的 v1.5 最小變更方案，並在作答頁加入**課次選擇器**。
...
- 作答頁可切換「全部課次 / 第 01 課 / 第 02 課...」
- 返回首頁按鈕置於書寫區內右上角的低干擾位置
- 筆順提示只播放**未完成格**，提示結束後未完成格可繼續書寫
...
- 本次只調整作答頁課次篩選、提示修補與對應文件；schema 與匯入規則不變
