...
""",
'known vocabulary rendering')

start = s.index('  function createWriterState(question, token, index) {')
end = s.index('\n  async function recordWriterResult', start)
new_block = r'''  function createWriterState(question, token, index) {
...
""",
'manual hint first unfinished writer only')
...
""",
'loading element reference')

app.write_text(s, encoding='utf-8')

index = Path('index.html')
h = index.read_text(encoding='utf-8')
old = '<section class="card question-card"><div class="question-instruction">請先聽整句，再把空格中的字寫在下方。</div><div id="question-text" class="question-text">準備中…</div></section>'
new = '<section class="card question-card"><div class="question-instruction">請先聽整句，再把空格中的字寫在下方。</div><div id="question-loading" class="question-loading hidden" role="status">載入中…</div><div id="question-text" class="question-text">準備中…</div></section>'
if h.count(old) != 1:
    raise SystemExit(f'question loading UI: expected 1 match, got {h.count(old)}')
index.write_text(h.replace(old, new, 1), encoding='utf-8')

css = Path('assets/app.css')
c = css.read_text(encoding='utf-8')
marker = '/* quiz-ux-behavior-fix */'
if marker not in c:
    c += '''\n\n/* quiz-ux-behavior-fix */
.known-lesson-vocab {
  color: #dc2626;
  font-weight: 800;
}
